...
<#
.SYNOPSIS
    Provisions the minimal Azure resources for the regional and multilingual
    experience PoC: a resource group, an Azure App Configuration store (Free),
    and an Azure OpenAI account with one chat model deployment. Assigns Entra ID
    (RBAC) roles to the signed-in user and writes ../.env.
...
.NOTES
    ...
    This is the same minimal footprint as proof of concept 001. Markets are
    labels inside one store, not extra resources, so serving three markets costs
    no more here than serving one.
#>
[CmdletBinding()]
param(
    ...
    [string]$BaseName = "regml$(Get-Random -Maximum 9999)",
...
$tags = @(
    "Application=regional-multilingual-experience",
    "Environment=poc",
    "Owner=$userUpn",
